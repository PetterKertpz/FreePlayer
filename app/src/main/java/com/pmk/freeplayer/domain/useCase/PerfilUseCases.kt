...
class ObtenerPerfilUseCase @Inject constructor(private val repository: UsuarioRepository) {
...
class ActualizarNombreUseCase @Inject constructor(private val repository: UsuarioRepository) {
  suspend operator fun invoke(nombre: String): Result<Unit> {
    return try {
      ...
      repository.actualizarNombrePerfil(nombre.trim())
      Result.success(Unit)
    } catch (e: Exception) {
      Result.failure(e)
    }
...
class ActualizarAvatarUseCase @Inject constructor(private val repository: UsuarioRepository) {
  suspend operator fun invoke(uri: String?): Result<Unit> {
    return try {
      repository.actualizarAvatarPerfil(uri)
      Result.success(Unit)
    } catch (e: Exception) {
      Result.failure(e)
    }
...
@Inject
constructor(private val repository: UsuarioRepository) {
...
constructor(private val repository: UsuarioRepository) {
  suspend operator fun invoke(genre: Genre): Result<Unit> {
    return try {
      repository.actualizarGeneroFavorito(genre)
      Result.success(Unit)
    } catch (e: Exception) {
      Result.failure(e)
    }
...
constructor(private val repository: UsuarioRepository) {
  suspend operator fun invoke(): Result<Unit> {
    return try {
      repository.reiniciarEstadisticasPerfil()
      Result.success(Unit)
    } catch (e: Exception) {
      Result.failure(e)
    }
