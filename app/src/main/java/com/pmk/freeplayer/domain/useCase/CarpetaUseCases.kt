...
class ObtenerTodasCarpetasUseCase @Inject constructor(
	...
	operator fun invoke(): Flow<List<Carpeta>> = repository.obtenerTodasLasCarpetas()
}

class ObtenerCarpetasVisiblesUseCase @Inject constructor(
	private val repository: BibliotecaRepository
) {
	operator fun invoke(): Flow<List<Carpeta>> = repository.obtenerCarpetasVisibles()
}

class ObtenerCarpetasOcultasUseCase @Inject constructor(
	private val repository: BibliotecaRepository
) {
	operator fun invoke(): Flow<List<Carpeta>> = repository.obtenerCarpetasOcultas()
}
