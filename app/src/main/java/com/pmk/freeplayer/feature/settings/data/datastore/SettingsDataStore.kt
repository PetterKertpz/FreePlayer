...
constructor(
      ...
      get() {
         ...
         return datastoreCache.getOrPut(userId) {
            PreferenceDataStoreFactory.create(
               produceFile = { context.preferencesDataStoreFile("settings_prefs_$userId") }
            )
         }
      }
   ...
   private object Keys {
      ...
      val EXCLUDED_PATHS = stringPreferencesKey("excluded_paths") // serializado como CSV
   }
      ...
      get() =
         activeDataStore.data.map { p ->
            ...
            UserPreferences(
               ...
               defaultRepeatMode =
                  p[Keys.DEFAULT_REPEAT_MODE]?.toEnum<RepeatMode>() ?: d.defaultRepeatMode,
               defaultShuffleEnabled = p[Keys.DEFAULT_SHUFFLE_ENABLED] ?: d.defaultShuffleEnabled,
               gaplessPlaybackEnabled =
                  p[Keys.GAPLESS_PLAYBACK_ENABLED] ?: d.gaplessPlaybackEnabled,
               crossfadeDurationSeconds =
                  p[Keys.CROSSFADE_DURATION_SECONDS] ?: d.crossfadeDurationSeconds,
               resumeOnStart = p[Keys.RESUME_ON_START] ?: d.resumeOnStart,
               equalizerEnabled = p[Keys.EQUALIZER_ENABLED] ?: d.equalizerEnabled,
               equalizerPreset =
                  p[Keys.EQUALIZER_PRESET]?.toEnum<EqualizerPreset>() ?: d.equalizerPreset,
               ...
               sortConfig =
                  SortConfig(
                     ...
                     direction =
                        p[Keys.SORT_DIRECTION]?.toEnum<SortDirection>() ?: d.sortConfig.direction,
                  ),
               excludedPaths =
                  p[Keys.EXCLUDED_PATHS]?.split("|")?.filter { it.isNotBlank() }?.toSet()
                     ?: d.excludedPaths,
               ...
               showLockScreenControls =
                  p[Keys.SHOW_LOCK_SCREEN_CONTROLS] ?: d.showLockScreenControls,
               defaultSleepTimerMinutes =
                  p[Keys.DEFAULT_SLEEP_TIMER_MINUTES] ?: d.defaultSleepTimerMinutes,
            )
         }
   ...
   suspend fun setDefaultRepeatMode(mode: RepeatMode) = edit {
      it[Keys.DEFAULT_REPEAT_MODE] = mode.name
   }

   suspend fun setDefaultShuffleEnabled(v: Boolean) = edit { it[Keys.DEFAULT_SHUFFLE_ENABLED] = v }

   suspend fun setGaplessPlaybackEnabled(v: Boolean) = edit {
      it[Keys.GAPLESS_PLAYBACK_ENABLED] = v
   }

   suspend fun setCrossfadeDuration(seconds: Int) = edit {
      it[Keys.CROSSFADE_DURATION_SECONDS] = seconds
   }
   ...
   suspend fun setBassBoostLevel(level: Int) = edit {
      it[Keys.BASS_BOOST_LEVEL] = level.coerceIn(0, 100)
   }

   suspend fun setVirtualizerLevel(level: Int) = edit {
      it[Keys.VIRTUALIZER_LEVEL] = level.coerceIn(0, 100)
   }

   suspend fun setPlaybackSpeed(speed: Float) = edit {
      it[Keys.PLAYBACK_SPEED] = speed.coerceIn(0.5f, 2.0f)
   }
   ...
   suspend fun setGridColumns(columns: Int) = edit {
      it[Keys.GRID_COLUMNS] = columns.coerceIn(1, 4)
   }

   suspend fun setMinDurationSeconds(seconds: Int) = edit {
      it[Keys.MIN_DURATION_SECONDS] = seconds.coerceAtLeast(0)
   }
	
	suspend fun addExcludedPath(path: String) = edit { prefs ->
		val current = prefs[Keys.EXCLUDED_PATHS]
			?.split("|")
			?.filter { it.isNotBlank() }
			?.toMutableSet()
			?: mutableSetOf()
		current.add(path)
		prefs[Keys.EXCLUDED_PATHS] = current.joinToString("|")
	}
	
	suspend fun removeExcludedPath(path: String) = edit { prefs ->
		val updated = prefs[Keys.EXCLUDED_PATHS]
			?.split("|")
			?.filter { it.isNotBlank() && it != path }
			?: emptyList()
		prefs[Keys.EXCLUDED_PATHS] = updated.joinToString("|")
	}
   ...
   suspend fun setShowLockScreenControls(v: Boolean) = edit {
      it[Keys.SHOW_LOCK_SCREEN_CONTROLS] = v
   }

   // ── Sleep timer ───────────────────────────────────────────────

   suspend fun setDefaultSleepTimerMinutes(minutes: Int) = edit {
      it[Keys.DEFAULT_SLEEP_TIMER_MINUTES] = minutes.coerceAtLeast(1)
   }
