...
interface ArtistDao {
   ...
   @Insert(onConflict = OnConflictStrategy.Companion.REPLACE) suspend fun insert(artist: ArtistEntity): Long
   ...
   @Query("DELETE FROM artists WHERE artist_id = :id") suspend fun deleteById(id: Long)
   ...
   @Query("SELECT COUNT(*) FROM artists") fun getArtistCount(): Flow<Int>
   ...
   /**
    * Recalcula las estadísticas de un artista específico basándose en las canciones. IMPORTANTE:
    * 'songs' debe tener 'artist_id' y 'album_id'.
    */
   @Query(
      """
        UPDATE artists
   ...
   /** Recálculo masivo para toda la biblioteca. Útil después de un escaneo completo. */
   @Transaction
   ...
   @Query("UPDATE artists SET total_songs = 0, total_albums = 0") suspend fun resetCounters()
   ...
   /**
    * Método transaccional para el Escáner: Busca el artista por nombre. Si no existe, lo crea.
   ...
   suspend fun getOrCreateArtistId(name: String): Long {
      ...
      return if (existing != null) {
         ...
         val newArtist = ArtistEntity(name = name, dateAdded = System.currentTimeMillis())
         insert(newArtist)
      }
   }

   @Query("SELECT COUNT(*) FROM artists") suspend fun getTotalArtistsCount(): Int

   @Query("UPDATE artists SET is_favorite = NOT is_favorite WHERE artist_id = :artistId")
   suspend fun toggleFavorite(artistId: Long)

   @Query("UPDATE artists SET website_url = :websiteUrl WHERE artist_id = :artistId")
   suspend fun updateSocialLinks(artistId: Long, websiteUrl: String?)
}