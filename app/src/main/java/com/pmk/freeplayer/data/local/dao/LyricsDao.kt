...
interface LyricsDao {
   ...
   @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(lyrics: LyricsEntity): Long

   @Insert(onConflict = OnConflictStrategy.REPLACE)
   suspend fun insertAll(lyricsList: List<LyricsEntity>)

   @Update suspend fun update(lyrics: LyricsEntity)
   ...
   @Delete suspend fun delete(lyrics: LyricsEntity)

   @Query("DELETE FROM lyrics WHERE song_id = :songId") suspend fun deleteBySongId(songId: Long)
   ...
   /**
    * Busca dentro del contenido de la letra. Útil para "Sé que la canción dice... pero no sé el
    * título".
    */
   @Query(
      """
   ...
   @Query("SELECT COUNT(*) FROM lyrics") suspend fun getLyricsCount(): Int

   @Query("SELECT EXISTS(SELECT 1 FROM lyrics WHERE song_id = :songId)")
   suspend fun hasLyrics(songId: Long): Boolean

   @Query("SELECT COUNT(*) FROM lyrics WHERE is_synced = 1") suspend fun getSyncedLyricsCount(): Int

   /**
    * Encuentra canciones que NO tienen letra. Útil para el worker de "Descargar letras faltantes".
    */
   @Query(
      """
        SELECT s.song_id 
   ...
   /**
    * Estadísticas por fuente (Genius, Manual, etc.) Mapeado al DTO genérico 'TopItemDto' para
